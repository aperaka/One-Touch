...
async function leagueMembers(req,res,id){try{const user=await requireUser(req,res);if(!user)return;if(!pool)return json(res,503,{error:'Leagues require the PostgreSQL database.'});const access=await pool.query('SELECT 1 FROM league_members WHERE league_id=$1 AND user_id=$2',[id,user.id]);if(!access.rowCount)return json(res,403,{error:'You are not a member of this league.'});const r=await pool.query(`SELECT u.id,u.display_name,lm.joined_at,lm.total_points,lm.wins,lm.matches_played,(l.owner_user_id=u.id) AS is_owner FROM league_members lm JOIN users u ON u.id=lm.user_id JOIN leagues l ON l.id=lm.league_id WHERE lm.league_id=$1 ORDER BY lm.total_points DESC,lm.wins DESC,lm.matches_played DESC,lm.joined_at ASC`,[id]);json(res,200,{members:r.rows.map((x,i)=>({rank:i+1,id:x.id,displayName:x.display_name,joinedAt:x.joined_at,isOwner:x.is_owner,totalPoints:Number(x.total_points||0),wins:Number(x.wins||0),matchesPlayed:Number(x.matches_played||0)}))})}catch(e){json(res,500,{error:'Could not load league members.'})}}
